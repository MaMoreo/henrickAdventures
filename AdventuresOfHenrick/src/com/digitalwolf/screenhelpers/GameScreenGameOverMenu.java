...
public class GameScreenGameOverMenu extends GameScreenAbstractMenu{
	...
	float dipRatioWidth = 80 * AppSettings.getWorldSizeRatio();  //80
	float dipRatioHeight = 80 * AppSettings.getWorldSizeRatio();  //80
	...
	public void setUpMenu(final GameScreen gameScreen) {
		...
		infoButton = MenuCreator.createCustomGameButton(Assets.bigFont,
				Assets.pummaIcon, Assets.pummaIcon, 100, 100, true);//dipRatioWidth/2, dipRatioHeight, true);	
	...
	public void sendInMenu(GameScreen gameScreen) {
		...
		gameOverMenuTable.addAction(Actions.moveTo(0, 0, 0.3f));
	}

	private void setUpMusic() {
		if (GameData.getSoundEnabled()) {
			Assets.musicGameOver.play();
		}
	}
	...
	public void sendAwayMenu(GameScreen gameScreen) {
		...
		gameOverMenuTable.addAction(Actions.moveTo(0,- gameOverMenuTable.getHeight(), 0.3f));
	}
	
	/**
	 * Para la musica para esta fase;
	 */
